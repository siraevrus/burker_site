...
# API парсинга каталога (https://parcing.burker-watches.ru) — заголовок X-API-KEY
API_JSON_KEY="ваш-ключ-из-auth_config-парсинга"
...
### База данных (автоматически через cron)

Скрипт `scripts/backup-database.sh` создаёт бэкапы каждые 30 минут и хранит их 1 неделю.

**Добавить в crontab:**
```bash
crontab -e
# Добавить строку:
*/30 * * * * /var/www/burker-watches.ru/scripts/backup-database.sh > /dev/null 2>&1
```

Бэкапы сохраняются в `backups/` (dev.db.backup.YYYYMMDD_HHMMSS). Старые удаляются автоматически.

**Ручное копирование (при необходимости):**
```bash
