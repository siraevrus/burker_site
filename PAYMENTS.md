...
Для проверки интеграции (в т.ч. тест №7 — формирование чека):

1. **Терминал** — в `TBANK_TERMINAL` должно быть слово **DEMO** (например `1234567890DEMO`). Учётные данные берутся в ЛК интернет-эквайринга (тестовый режим).
2. При DEMO автоматически используется тестовый URL `https://rest-api-test.tinkoff.ru` (если не задан `TBANK_EACQ_BASE_URL`).
3. В Init передаётся объект **Receipt** (Items, Email, Taxation, Payments) — для формирования чека.
4. **Тестовая карта** для успешной оплаты: уточните в [документации T-Bank](https://developer.tbank.ru/eacq) или ЛК (например `4200 0000 0000 0000` и аналогичные).

### Порядок проверки

1. Убедитесь, что `TBANK_TERMINAL` содержит **DEMO**.
2. Инициируйте платёж (оформление заказа или `npm run test:tbank`) — Receipt передаётся автоматически.
3. Оплатите тестовой картой.
4. В ЛК Т-Бизнес → **Операции** проверьте, что статус операции «Подтверждён».
5. Тест №7 — статус должен быть «Пройден».

Запуск скрипта: `npm run test:tbank` (создаёт ссылку на 10 ₽ с Receipt).
