...
if [[ -n "$DIRTY" ]]; then
  ...
  while IFS= read -r line; do
    path="${line:3}"
    case "$path" in
      public/promo/*|public/products/*|public/order-proofs/*|uploads/*|uploads/promo/*|public/yandex_*.html|ecosystem.config.js|backups|backups/|backups/*|orange_prod/*|.duckversions/|.duckversions/*|public/.duckversions/|public/.duckversions/*) ;;
...
if [[ -d "public/products" ]] || [[ -d ".next/standalone/public/products" ]]; then
  ...
  # Копируем из public/products (приоритет — туда пишет upload-image)
  if [[ -d "public/products" ]]; then
    cp -n public/products/* "${PRODUCTS_BACKUP}/products/" 2>/dev/null || true
  fi
  # Дополняем файлами из standalone (на случай если были загружены до фикса путей)
