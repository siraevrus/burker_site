...
Если проект развернут на **Vercel**, cron уже настроен через файл `vercel.json`. После деплоя он будет работать автоматически.

**Проверка:** После деплоя зайдите в Vercel Dashboard → Settings → Cron Jobs и убедитесь, что задача создана.
...
```bash
CRON_SECRET_KEY=ваш-секретный-ключ-здесь
```
