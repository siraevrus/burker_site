# Настройка cron: импорт товаров, обновление курсов валют, бэкап БД
...
- **Бэкап базы данных** — каждые 30 минут (скрипт `scripts/backup-database.sh`), срок хранения 1 неделя.
...
- **`API_JSON_KEY`** — обязательна для импорта: ключ авторизации API каталога (https://parcing.burker-watches.ru). Передаётся в заголовке `X-API-KEY`.
...
**Бэкап с логом:**
```bash
*/30 * * * * BACKUP_LOG=/var/log/db-backup.log /var/www/burker-watches.ru/scripts/backup-database.sh 2>&1
```
...
**Бэкап базы данных** (`scripts/backup-database.sh`):
- `*/30 * * * *` = каждые 30 минут
- Бэкапы хранятся в `backups/`, автоматически удаляются старше 7 дней
