...
**Расписание:** раз в сутки в 05:00 UTC. Для 05:00 по Москве (МСК) используйте **02:00 UTC**: `"0 2 * * *"` в настройках cron (crontab, cron-job.org и т.п.).

**Ручной вызов (с секретом):**
```bash
# Вариант 1: заголовок (crontab и др.)
...
- **`CRON_SECRET`** или **`CRON_SECRET_KEY`** — секретный ключ для авторизации cron-вызовов.
...
### 2. Настройка через внешние сервисы
...
### 3. Настройка на собственном сервере (Linux)
...
1. **Внешний cron:** Логи на сайте сервиса (cron-job.org и т.п.)
2. **Собственный сервер:** Логи приложения или `/var/log/`
